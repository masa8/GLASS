...
from commands.net.glass import GLASS
...
class GLASS_onnx(torch.nn.Module):
    def __init__(self):
        ...
        self.backbone.load_state_dict(
            torch.load(
                "/root/.cache/torch/hub/checkpoints/wide_resnet50_2-95faca4d.pth",
                map_location="cuda",
            )
        )
        self.glass = GLASS("cuda")
        self.glass.load(
            self.backbone, ["layer2", "layer3"], "cuda", (3, 288, 288), 1536, 1536
        )
        state_dict = torch.load(
            "../results/models/backbone_0/wfdd_grid_cloth/ckpt_best_219.pth",
            map_location="cuda",
        )
    ...
    def forward(self, img):
        with torch.no_grad():
            patch_features, patch_shapes = self.glass._embed(
            ...
            patch_scores = self.glass.patch_maker.unpatch_scores(
...
if __name__ == "__main__":
    ...
    onnx.save(
        model_simp, "../results/models/backbone_0/wfdd_grid_cloth/glass_simplified.onnx"
    )
